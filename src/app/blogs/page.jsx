...
const BlogPage = async () => {
	...
	return (
		<div className="container mx-auto px-4 py-12 mt-16 space-y-12">
			<Breadcrumbs breadcrumbs={BREADCRUMBS} />

			{/* Featured Post */}
			<article className="hero bg-base-100 rounded-3xl shadow-xl border border-base-300">
				<div className="hero-content flex-col lg:flex-row gap-8 p-8">
					<figure className="lg:w-1/2">
						<Link href={`/blogs/${firstPost.slug}`}>
							<Image
								className="rounded-2xl w-full h-64 lg:h-80 object-cover hover:scale-105 transition-transform duration-300"
					...
					<div className="lg:w-1/2 space-y-4">
						<div className="flex items-center justify-between flex-wrap gap-2">
							<time className="text-sm font-medium text-primary uppercase tracking-wide">
						...
						<Link
							...
							className="block text-2xl lg:text-3xl font-bold leading-tight hover:text-primary transition-colors duration-200"
						>
							{firstPost.title}
						</Link>
						...
						<div className="flex flex-wrap gap-2">
							{firstPost.tags?.map((tag, index) => (
								<div
									key={index}
									className="badge badge-outline gap-1 hover:badge-primary transition-colors"
								>
